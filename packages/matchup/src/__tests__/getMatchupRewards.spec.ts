...
describe('getMatchupRewards', () => {
  ...
  test('should distribute rewards correctly for top 3 distinct scores', async () => {
    ...
    expect(rewards).toEqual([
      { address: '0x1', scoutId: 'scout1', pointsAmount: 500, opAmount: parseUnits('60', optimismTokenDecimals) }, // 50%
      { address: '0x2', scoutId: 'scout2', pointsAmount: 300, opAmount: parseUnits('25', optimismTokenDecimals) }, // 30%
      { address: '0x3', scoutId: 'scout3', pointsAmount: 200, opAmount: parseUnits('15', optimismTokenDecimals) } // 20%
    ]);
  });
  ...
  test('should distribute rewards correctly with a tie for 1st place', async () => {
    ...
    expect(rewards).toContainEqual({
      address: '0x1',
      scoutId: 'scout1',
      pointsAmount: expectedPointsSplit,
  ...
  test('should distribute rewards correctly with a tie for 2nd place', async () => {
    ...
    const expectedPointsSplit = Math.floor((mockMatchupPool * 0.5) / 2); // 250
  ...
  test('should distribute rewards correctly with a tie for 3rd place', async () => {
    ...
    const expectedPointsSplit = Math.floor((mockMatchupPool * 0.2) / 2); // 100
  ...
  test('should distribute rewards correctly with a three-way tie for 1st place', async () => {
    ...
    const expectedPointsSplit = Math.floor(mockMatchupPool / 3); // 333
  ...
  test('should skip participants without a primary wallet', async () => {
    ...
    expect(rewards).toHaveLength(2);
    expect(rewards).toEqual([
      { address: '0x1', scoutId: 'scout1', pointsAmount: 500, opAmount: parseUnits('60', optimismTokenDecimals) }, // 1st
      { address: '0x3', scoutId: 'scout3', pointsAmount: 200, opAmount: parseUnits('15', optimismTokenDecimals) } // 2nd (scout2 skipped)
    ]);
  });
  ...
  test('should handle large numbers and potential floating point inaccuracies for points', async () => {
    ...
    const expectedPoints = Math.floor(largePool / 3); // floor(9999 / 3) = 3333
